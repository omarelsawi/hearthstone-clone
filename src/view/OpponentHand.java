...
public class OpponentHand extends JPanel implements MouseListener{
	...
	public OpponentHand(Game g) {
		
		    oh=new ArrayList<JLabel>();
	        this.p=g.getCurrentHero();
	        this.p2=g.getOpponent();
	        error=new JTextField();
	        ok=new JButton("Okay!");
		    this.setBounds(200, 400, 1000, 300);
		    int y=0;
	          for(int i=0;i<g.getOpponent().getHand().size();i++){
			        
			    	String s="Images/cardback.png";
			    	JLabel c =new JLabel(new ImageIcon(s));
			    	c.setBounds(300+y,200,116,200);
			    	c.setSize(110, 200);
			    	c.setVisible(true);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(p.getHand().get(r) instanceof Minion){
			...
			if( p.getHand().get(r) instanceof FieldSpell)
				...
				try {
					p.castSpell((LeechingSpell)p.getHand().get(r), field2.getSelectedminion());
				} catch (NotYourTurnException e1) {
					error=new JTextField("wait for your turn");
					error.setEditable(false);
					error.setVisible(true);
					...
					try {
						p.castSpell((LeechingSpell)p.getHand().get(r), field1.getSelectedminion());
					} catch (NotYourTurnException e1) {
						error=new JTextField("wait for your turn");
						error.setEditable(false);
						error.setVisible(true);
